...
export default function Sidebar({
  ...
  const renderChatRow = (chat: any, isPinned: boolean) => (
    <div
      key={chat.id}
      className={`group relative rounded-2xl border transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ${
    ...
    >
      <button
        ...
        className={`w-full truncate py-2.5 pl-3 pr-12 text-left text-sm transition-colors duration-200 ${
          activeChatId === chat.id
            ? "text-white/92"
            : isPinned
      ...
      <button
        ...
        className={`absolute right-1.5 top-1.5 flex h-8 w-8 items-center justify-center rounded-xl transition-[background-color,border-color,color,opacity,transform] duration-200 hover:bg-white/[0.07] hover:text-white active:scale-95 ${
      ...
      {openChatMenuId === chat.id && (
        <div className="absolute right-2 top-11 z-50 min-w-[152px] overflow-hidden rounded-2xl border border-white/8 bg-[#0f1728]/96 shadow-[0_20px_48px_rgba(0,0,0,0.34)] backdrop-blur-2xl animate-[fadeInUp_0.18s_ease-out]">
          <button
            ...
            className="w-full px-3 py-2.5 text-left text-sm text-white/82 transition-[background-color,color] duration-200 hover:bg-white/[0.06] hover:text-white"
          >
            {isPinned ? "Unpin" : "Pin"}
          </button>
          ...
          <button
            ...
            className="w-full px-3 py-2.5 text-left text-sm text-red-300/90 transition-[background-color,color] duration-200 hover:bg-white/[0.06] hover:text-red-200"
          >
            Delete
          </button>
        </div>
    ...
    >
      <div className="flex h-full flex-col">
        <button
          ...
          className="mb-3 rounded-2xl bg-gradient-to-r from-[#1d4ed8] to-[#3b82f6] px-3 py-2.5 text-sm font-medium text-white shadow-[0_10px_24px_rgba(59,130,246,0.24)] transition-[transform,filter,box-shadow] duration-200 hover:brightness-110 active:scale-[0.97]"
        >
          + New Chat
        </button>
        ...
        <input
          ...
          className="mb-3 rounded-2xl border border-white/8 bg-white/[0.04] px-3 py-2.5 text-sm text-white/88 outline-none placeholder:text-white/28 transition-[background-color,border-color,color,box-shadow] duration-200 focus:border-white/14 focus:bg-white/[0.06]"
        />
            ...
            <div className="space-y-1.5">
              {archivedChats.length > 0 ? (
                archivedChats.map((chat) => (
                  <div
                    key={chat.id}
                    className="group flex items-center justify-between rounded-2xl border border-white/8 bg-white/[0.025] px-3 py-2.5 text-sm transition-[background-color,border-color,color,transform] duration-200 hover:-translate-y-[1px] hover:border-white/10 hover:bg-white/[0.045]"
                  >
                    ...
                    <button
                      ...
                      className="rounded-full border border-[#3b82f6]/20 bg-[#3b82f6]/8 px-3 py-1 text-xs text-white/70 transition-[background-color,border-color,color,transform] duration-200 hover:border-[#3b82f6]/36 hover:bg-[#3b82f6]/12 hover:text-white active:scale-95"
                    >
                      Restore
                    </button>
                  </div>
            ...
            <div className="space-y-1.5">
              {deletedChats.length > 0 ? (
                deletedChats.map((chat) => (
                  <div
                    key={chat.id}
                    className="group flex items-center justify-between rounded-2xl border border-red-400/10 bg-red-500/[0.04] px-3 py-2.5 text-sm transition-[background-color,border-color,color,transform] duration-200 hover:-translate-y-[1px] hover:border-red-400/16 hover:bg-red-500/[0.07]"
                  >
        ...
        <div className="mt-4 space-y-2 border-t border-white/8 pt-3.5">
          <button
            ...
            className="w-full rounded-2xl border border-white/8 bg-white/[0.04] px-3 py-2.5 text-left text-sm text-white/82 transition-[background-color,border-color,color,transform] duration-200 hover:-translate-y-[1px] hover:border-white/12 hover:bg-white/[0.06] hover:text-white active:scale-[0.98] active:scale-[0.98] active:scale-[0.98] active:scale-[0.98]"
          >
            Feedback / Idea
          </button>
          ...
          {!isPersonalRoute && (
            <button
              ...
              className="w-full rounded-2xl border border-white/8 bg-white/[0.04] px-3 py-2.5 text-left text-sm text-white/82 transition-[background-color,border-color,color,transform] duration-200 hover:-translate-y-[1px] hover:border-white/12 hover:bg-white/[0.06] hover:text-white active:scale-[0.98] active:scale-[0.98] active:scale-[0.98]"
            >
              Log in
            </button>
          )}
